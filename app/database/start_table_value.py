    ...
    try:
        if not db.query(crud.models.Ship).first():
            ships = [
                ...
                schemas.ShipCreate(ship_name="Condor", attack=30, shield=25, evasion=20, fire_rate=5, hp=200, value=3000),
                schemas.ShipCreate(ship_name="Vulture", attack=35, shield=30, evasion=25, fire_rate=6, hp=220, value=3500)
            ]
            for ship in ships:
...
def seed_users(db: Session) -> dict:
    ...
    try:
        if not db.query(crud.models.User).first():
            users = [
                schemas.UserCreate(nickname="Admin", rank_elo=1000, currency_value=9999999),
